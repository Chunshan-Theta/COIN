<?php
    ...
    switch($action){
        case 'login':       // --------------------------登入系統----------------------
            ...
            $sql = "SELECT * FROM `userinfo` WHERE `account` = '".mysql_real_escape_string($account)."' AND `password` = '".mysql_real_escape_string($password)."' AND `identity` = '".mysql_real_escape_string($identity)."'";
            ...
            }else{  exit('{"check":"illegal", "announcement":"【警告】無此帳號，請確認輸入內容是否正確！"}');
            }
